...
describe('generate', () => {
  ...
  describe('default mode (strictSync: false)', () => {
    ...
    it('should print orphaned errors only for langs that have orphaned keys', () => {
      writeJson(tmpDir, 'i18n/core/en.json', { title: 'Hello', key2: 'X' });
      writeJson(tmpDir, 'i18n/core/es.json', { title: 'Hola' });
      writeJson(tmpDir, 'i18n/core/fr.json', { title: 'Bonjour', key2: 'Y', extra: 'E' });

      const success = generate(
        tmpDir,
        makeOptions(tmpDir, { supportedLangs: ['en', 'es', 'fr'] }),
      );
