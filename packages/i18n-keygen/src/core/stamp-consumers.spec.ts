...
describe('stampConsumerFiles', () => {
  ...
  describe('direct consumers', () => {
    ...
    it('should handle stamp at the first line of the file', () => {
      writeFile(tmpDir, 'i18n/keys.ts', 'export type I18nKey = never;\n');
      writeFile(
        tmpDir,
        'i18n/pipe.ts',
    ...
    it('should handle comment without blank line above it', () => {
      writeFile(tmpDir, 'i18n/keys.ts', 'export type I18nKey = never;\n');
      writeFile(
        tmpDir,
        'i18n/pipe.ts',
    ...
    it('should handle bare stamp with blank line above it', () => {
      writeFile(tmpDir, 'i18n/keys.ts', 'export type I18nKey = never;\n');
      writeFile(
        tmpDir,
        'i18n/pipe.ts',
  ...
  describe('transitive consumers', () => {
    ...
    it('should skip non-relative imports when checking transitive consumers', () => {
      writeFile(tmpDir, 'i18n/keys.ts', 'export type I18nKey = never;\n');
      writeFile(
        tmpDir,
        'i18n/pipe.ts',
    ...
    it('should handle unreadable directories during transitive scan', () => {
      writeFile(tmpDir, 'i18n/keys.ts', 'export type I18nKey = never;\n');
      writeFile(
        tmpDir,
        'i18n/pipe.ts',
      ...
      fs.mkdirSync(path.join(tmpDir, 'restricted'), { recursive: true });
      fs.chmodSync(path.join(tmpDir, 'restricted'), 0o000);

      expect(() => {
        stampConsumerFiles(path.join(tmpDir, 'i18n/keys.ts'), 'abc123');
      }).not.toThrow();

      fs.chmodSync(path.join(tmpDir, 'restricted'), 0o755);
    });

    it('should resolve transitive imports that include .ts extension', () => {
      writeFile(tmpDir, 'i18n/keys.ts', 'export type I18nKey = never;\n');
      writeFile(
        tmpDir,
        'i18n/pipe.ts',
  ...
  describe('when consumer file has no trailing newline', () => {
    it('should add trailing newline after stamp', () => {
      writeFile(tmpDir, 'i18n/keys.ts', 'export type I18nKey = never;\n');
      writeFile(
        tmpDir,
        'i18n/pipe.ts',
